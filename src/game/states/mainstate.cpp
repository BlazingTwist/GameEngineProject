...
namespace gameState {
    ...
    static void printInfo() {
        ...
        spdlog::info("- Press [6] to enter a new main state");
        spdlog::info("- Press [7] to exit this state");
    }
    ...
    void MainGameState::initializeControls() {
        ...
        hotkey_mainState_isDown = input::InputManager::isKeyPressed(input::Key::Num6);
        hotkey_exit_isDown = input::InputManager::isKeyPressed(input::Key::Num7);
    }
    ...
    void MainGameState::update(const long long int &deltaMicroseconds) {
        ...
        if (!hotkey_mainState_isDown && input::InputManager::isKeyPressed(input::Key::Num6)) {
            spdlog::info("main state -> main state");
            gameState::GameStateManager::getInstance().startGameState(new MainGameState());
            return;
        }

        if (!hotkey_exit_isDown && input::InputManager::isKeyPressed(input::Key::Num7)) {
