...
namespace smk {
    ...
    {
    public:
        Access(ComponentAccess<Component>& _storage) noexcept
                : m_targetStorage(_storage)
        {}

        // Retrieve a component of _ent.
        // @return A pointer to the associated component or nullptr if it does not exist.
        Component* at(Entity _ent) { return m_targetStorage.at(_ent); }
        const Component* at(Entity _ent) const { return m_targetStorage.at(_ent); }

        template<typename... Args>
        Component& insert(Entity _ent, Args&&... _args)
        {
            return m_targetStorage.insert(_ent, std::forward<Args>(_args)...);
        }
        ...
        void erase(Entity _ent) { m_targetStorage.erase(_ent); }
    public:
        ComponentAccess<Component>& m_targetStorage;
    };
    ...
    public:
        ...
        template <component_type Component>
        Access<Component> getComponents() {
        ...
        template <component_type Component>
        const Access<Component> getComponents() const {
        ...
        void execute(const Action& _action) {
            ...
            for (int i = startIndex + 1; i < static_cast<int>(componentTypes.size()); i++) {
                ...
                /*   for (int j = 0; j < entities.size(); j++) {
                       if (!componentsMap.at(componentTypes[i]).hasEntity(entities[j]))
                           entities.erase(next(entities.begin(),j));
                   */}
